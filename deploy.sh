...
SIMPLE_CREDS="${SIMPLE_CREDS:-true}"        # Default to simple mode (override with SIMPLE_CREDS=false)
USE_ENCRYPTION="${USE_ENCRYPTION:-false}"   # Optional basic encryption for production
...
Options:
  ...
  --simple-creds             Force simple credential mode (default: enabled)
  --secure-creds             Use encrypted credential storage for production
  --use-encryption           Enable basic encryption for credential files
...
Environment:
  ...
  SIMPLE_CREDS=true|false      (default true; false enables secure mode)
  USE_ENCRYPTION=true|false    (default false; true enables basic encryption)
...
handle_credentials() {
    ...
    if [[ "$SIMPLE_CREDS" == "true" ]]; then
        ...
        log_message INFO "Mode: Secure (legacy complex encryption - deprecated)"
        log_message WARN "Legacy secure mode is deprecated - use --use-encryption with simple mode instead"
    fi
    
    if (( SKIP_CREDS )); then
        log_message INFO "Skipping credential management (--skip-creds specified)"
        if [[ -z "$SPLUNK_PASSWORD" ]]; then
            log_message ERROR "Password required when skipping credential generation"
            log_message INFO "Either provide --splunk-password or set SPLUNK_PASSWORD environment variable"
            exit 1
        fi
        export SPLUNK_USER SPLUNK_PASSWORD
        log_message SUCCESS "Using provided credentials (user: $SPLUNK_USER)"
        return 0
    fi
    ...
    # Generate or prompt for new credentials
    log_message INFO "No existing credentials found - generating new credentials"
    
    if [[ -z "$SPLUNK_PASSWORD" ]]; then
        if (( NON_INTERACTIVE )); then
            log_message INFO "Non-interactive mode: Generating random password"
            SPLUNK_PASSWORD=$(generate_random_password)
            log_message INFO "Generated secure password (hidden from logs)"
        else
            ...
            while (( attempts < 3 )); do
                ...
                if (( attempts >= 3 )); then
                    error_exit "Too many failed password attempts"
                fi
        ...
        validate_password "$SPLUNK_PASSWORD" || error_exit "Provided password does not meet requirements"
    fi

    # Store the credentials
    store_credentials "$SPLUNK_USER" "$SPLUNK_PASSWORD"
    
    # Export for use by deployment
    export SPLUNK_USER SPLUNK_PASSWORD
    CREDENTIALS_GENERATED=1
    
    log_message SUCCESS "Credentials configured successfully"
    log_message INFO "User: $SPLUNK_USER"
    log_message INFO "Storage: ${CREDS_DIR}/ (mode: $([ "$USE_ENCRYPTION" == "true" ] && echo "encrypted" || echo "simple"))"
}
