...
SPLUNK_SECRET=\$(openssl rand -hex 16 2>/dev/null || echo "default-secret-key")
...
deploy_cluster() {
    ...
    if [[ ! -f "$BASE_COMPOSE" ]]; then
        ...
        if [[ -f "${SCRIPT_DIR}/lib/compose-generator.sh" ]]; then
            # Load all required core libraries first
            source "${SCRIPT_DIR}/lib/core.sh" || error_exit "Cannot load lib/core.sh"
            source "${SCRIPT_DIR}/lib/validation.sh" || log_message WARN "lib/validation.sh not available"
            source "${SCRIPT_DIR}/lib/error-handling.sh" || log_message WARN "lib/error-handling.sh not available"
            source "${SCRIPT_DIR}/lib/runtime-detection.sh" || log_message WARN "lib/runtime-detection.sh not available"
...
display_summary() {
    ...
    if (( WITH_MONITORING )); then
        log_message INFO "  Monitoring: enabled"
    else
...
setup_standard_logging "deploy-main"
