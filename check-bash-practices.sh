...
check_bash_practices() {
    ...
    local old_test_count
    old_test_count=$(grep -c "if \[ " "$file" 2>/dev/null || echo 0)
    local new_test_count
    new_test_count=$(grep -c "if \[\[" "$file" 2>/dev/null || echo 0)
    ...
    local unquoted_vars
    unquoted_vars=$(grep -c "sleep \$[A-Za-z_]\|echo \$[A-Za-z_]" "$file" 2>/dev/null || echo 0)
    if (( unquoted_vars == 0 )); then
        echo "  ✅ Variables properly quoted"
        ((practices_found++))
    else
        echo "  ⚠️  Some unquoted variables found ($unquoted_vars instances)"
    fi
